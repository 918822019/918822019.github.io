...
目的：验证 FSQ-8 (3-bit) 在 KL 蒸馏下的语义保持能力

核心观察点：
- 在纯 MSE 阶段（前 20%），FSQ-8 的误差可能会再次飙升到 20%+，不要慌，这是物理极限
- 盯紧 KL 散度：当进入 Alignment 阶段（KL 权重 0.7）后，观察 KL 散度是否能像上次一样，从高位被强行"拽"下来，稳定在 0.1 甚至 0.0X 的级别

预期目标：
- 如果 FSQ-8 的 KL 散度能降到 < 0.5，且最终 PPL 劣化 < 1.5，这就证明了 "KL 蒸馏成功在 3-bit 下挽救了语义"

技术改进（相对于 v1）：
1. Per-channel Salience（借鉴 AWQ 思想）
2. 延长 Warmup 阶段（前 20% 只用 MSE）
3. 梯度裁剪（max_norm=1.0）
4. 动态 Loss 退火策略
"""
...
class FSQQuantizer(nn.Module):
    """
    FSQ 量化器 v2（支持 Per-channel Salience 和 Outlier 保护）
    ...
    5. 支持 Outlier 混合精度保护（保护 top 1% 的异常值）
    """
    
    def __init__(self, levels: int = 8, group_size: int = 128, use_salience: bool = True, outlier_ratio: float = 0.01):
        super().__init__()
        self.levels = levels
        ...
        self.outlier_ratio = outlier_ratio  # Outlier 比例（默认 1%）
        self.outlier_mask = None  # Outlier 掩码
    
    def fit(self, x: torch.Tensor):
        """计算归一化参数，包括 Per-channel Salience 和 Outlier 保护"""
        ...
        # Outlier 保护机制：识别 top 1% 的异常值
        if x_scaled.dim() >= 2 and self.outlier_ratio > 0:
            # 计算绝对值分布
            abs_w = torch.abs(x_scaled)
            
            # 找出绝对值最大的前 1% 的权重
            threshold = torch.quantile(abs_w, 1.0 - self.outlier_ratio, dim=-1, keepdim=True)
            self.outlier_mask = abs_w > threshold
            
            # 将 outlier 置为 0，使其不影响 FSQ 的 scale 计算
            x_scaled = torch.where(self.outlier_mask, torch.zeros_like(x_scaled), x_scaled)
        else:
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Outlier 保护：反量化后，把 outlier 的原始值加回去
        if self.outlier_mask is not None:
            # 将 outlier 的原始值加回去
            x_quantized = torch.where(self.outlier_mask, x, x_quantized)
...
class DynamicLossScheduler:
    """
    动态 Loss 退火策略（FSQ-8 专用版）
    
    训练阶段（降低KL权重，保护权重不被撕裂）：
    - Warmup (前 20%): MSE=1.0, KL=0.0  # 让scale和min_val稳定
    - Alignment (20-60%): MSE=0.7, KL=0.3  # 大幅降低KL，保护权重
    - Fine-tune (60-100%): MSE=0.5, KL=0.5  # 两者平起平坐
    """
...
class LogitsKLDistillationLoss(nn.Module):
    ...
    关键改进：
    1. 温度 T 作为可学习参数（nn.Parameter），可被优化器更新
    2. 使用 log_T 确保 T > 0
    3. 支持 T^2 缩放梯度
    """
    ...
    def forward(
        ...
        if self.learnable_temperature:
            temperature = torch.exp(self.log_T)
        else:
        ...
        return total_loss, mse_loss, kl_loss, temperature.item()
...
def train_distillation_fsq8():
    ...
    """
    print("=" * 70)
    print("Qwen3.5-0.8B 模型蒸馏量化训练 - FSQ-8 (3-bit)")
    print("=" * 70)
    ...
    # 创建优化器（包含student和温度参数）
    optimizer = torch.optim.AdamW([
        {'params': student.parameters(), 'lr': 1e-4},
        {'params': loss_fn.parameters(), 'lr': 1e-3}  # 给温度一个独立的学习率
    ], weight_decay=0.01)
    ...
    # 训练循环
    print(f"\n开始训练（{total_steps} 步）...")
    print("核心观察点：在纯 MSE 阶段（前 20%），FSQ-8 的误差可能会再次飙升到 20%+，不要慌，这是物理极限")
    print("盯紧 KL 散度：当进入 Alignment 阶段（KL 权重 0.7）后，观察 KL 散度是否能从高位被强行'拽'下来")
    ...
    # 计算压缩比
    bits_per_element = np.log2(8)  # FSQ-8 (3-bit)
    ...
    try:
        ...
        print(f"码本利用率计算失败: {e}")
        gate_util = up_util = down_util = {'utilization': 0.0, 'used_levels': 0, 'total_levels': 8}
    ...
    print("\n保存训练结果...")
    try:
        output_file = output_dir / "distill_quantize_0_8b_fsq8_v2_results.txt"
    
        with open(output_file, "w", encoding="utf-8") as f:
        f.write("Qwen3.5-0.8B 蒸馏量化训练结果 - FSQ-8 (3-bit)\n")
        f.write("=" * 50 + "\n\n")
        ...
        f.write(f"  温度: 可学习参数（初始值 2.0）\n")
        f.write(f"  Outlier 保护: 启用（Per-channel Clipping，丢弃 1% 极端值）\n\n")
        
        f.write("Loss 退火策略（FSQ-8 专用版）:\n")
        f.write("  Warmup (前 20%): MSE=1.0, KL=0.0\n")
        f.write("  Alignment (20-60%): MSE=0.7, KL=0.3\n")
        f.write("  Fine-tune (60-100%): MSE=0.5, KL=0.5\n\n")
    ...
    try:
        quantized_weights = {
            ...
            'salience_gate': student.fsq_gate.salience_scale.cpu() if student.fsq_gate.salience_scale is not None else None,
            'salience_up': student.fsq_up.salience_scale.cpu() if student.fsq_up.salience_scale is not None else None,
            'salience_down': student.fsq_down.salience_scale.cpu() if student.fsq_down.salience_scale is not None else None,
        }
...
if __name__ == "__main__":
    # 运行训练
    losses = train_distillation_fsq8()