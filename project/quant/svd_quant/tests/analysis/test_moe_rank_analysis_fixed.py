...
def load_and_analyze_weights(file_path, target_keywords, max_samples=5):
    """加载并分析权重"""
    results = []
            ...
            for name in targets:
                try:
                    ...
                    if weight_np.ndim == 3:
                        ...
                        ranks = []
                        all_singular = []
                        for i in range(min(weight_np.shape[0], 10)):  # 只测试前10个专家
...
def main():
    ...
    # 分析关键层类型
    analysis_targets = [
        ("attention_k", "注意力K层", ["self_attn.k_proj"]),
        ("attention_v", "注意力V层", ["self_attn.v_proj"]),
        ("moe_gate_up_proj", "MoE专家Gate_Up层", ["mlp.experts.gate_up_proj"]),
        ("moe_down_proj", "MoE专家Down层", ["mlp.experts.down_proj"]),
        ("shared_gate_proj", "共享专家Gate层", ["shared_expert.gate_proj"]),
        ("shared_up_proj", "共享专家Up层", ["shared_expert.up_proj"]),
        ("shared_down_proj", "共享专家Down层", ["shared_expert.down_proj"]),
    ]
    ...
    for layer_type, description, keywords in analysis_targets:
        ...
        # 获取包含这些权重的文件
        files_to_load = set()
        for name in layers[layer_type][:5]:  # 只取前5个采样
        ...
        for file_name in files_to_load:
            ...
            results = load_and_analyze_weights(file_path, keywords, max_samples=3)
