...
【常用命令】
1. 爬取书籍目录：
    python -m src.crawler.engine crawl-books --start 1 --end 10000 --concurrency 5 --progress-every 100
    - --start, --end：指定书籍ID范围
    - --concurrency：并发数
    - --progress-every：每爬取多少本书输出一次进度

2. 爬取章节内容：
    python -m src.crawler.engine crawl-content --start 1 --end 10000 --concurrency 5 --batch-size 10 --chapter-progress-every 50 --max-pending-per-book 5
    - --batch-size：每次爬取的章节数
    - --chapter-progress-every：每爬取多少章节输出一次进度
    - --max-pending-per-book：每本书允许的最大未完成章节数

3. 导出分片：
    python -m src.crawler.engine export-shards --start 1 --end 10000 --output-dir ../data/shards --auto-continue
    - --output-dir：分片输出目录
...
【更多参数说明】
可通过 python -m src.crawler.engine --help 查看所有命令和参数。
====================
"""
...
from src.utils import now_iso as _now_iso  # noqa: E402
from src.utils import normalize_inline_text as _normalize_inline_text  # noqa: E402

BASE_SITE_URL = config.crawler.base_site_url  # type: ignore[union-attr]
...
DATA_DIR = os.path.normpath(os.path.join(BASE_DIR, "..", "..", config.data.dir))
DEFAULT_DB_PATH = os.path.join(DATA_DIR, config.data.db_name)
DEFAULT_SHARDS_DIR = os.path.join(DATA_DIR, config.data.shards_dir)
LOG_FILE = os.path.normpath(os.path.join(BASE_DIR, "..", "..", config.crawler.log_file))
...
def build_referer(url: str) -> str:
    """根据 API URL 中的 book_id / chapterid 构建 Referer 头。"""
...
class BookApiClient:
    """书籍 API 客户端，封装请求限流、重试、调试响应保存等逻辑。"""
    ...
    def _build_debug_file_prefix(self, url: str) -> str:
        """根据 URL 中的 book_id/chapter_id 构建调试文件前缀。"""
        parsed = parse.urlparse(url)
        query = parse.parse_qs(parsed.query)
        book_id = to_int((query.get("id") or ["0"])[0])
        chapter_id = to_int((query.get("chapterid") or ["0"])[0])
